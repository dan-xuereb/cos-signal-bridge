"""Bridge orchestration of composition across horizons (per D-13).

Wires the local compose() function into the bridge pipeline by extracting
signal values and IC weights from bridge-side data structures and passing
them to the pure composition engine.
"""
...
from .composite import CompositeScore, compose
...
def compose_signals(
    ...
    """Orchestrate composition across all horizons.
    ...
    Args:
        ...
        factor_ics: {signal_name: current_rolling_ic} — IC from FactorRecord.oos_monitoring.
            Values of None are treated as missing and excluded by compose().
        library: SignalLibrary with registered SignalMeta entries.
    ...
    results: dict[HorizonCategory, CompositeScore] = {}
    for h in horizons:
        score = compose(

...
from sdl.types import (
    ...
    _NORMALIZATION,
    _PRIMITIVE_OPS,
    _SINGLE_SERIES_TEMPORAL,
    _TWO_SERIES_TEMPORAL,
    _UNARY_ARITHMETIC,
    _UNARY_COMPARISON,
)
...
_DISPATCH[OperatorTag.sigmoid] = (
    lambda node, c, df: pd.Series(1.0 / (1.0 + np.exp(-c[0])), index=c[0].index)
)
...
def _clip_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
...
_DISPATCH[OperatorTag.roll_mean] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).mean()
)
_DISPATCH[OperatorTag.roll_std] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).std()
)
_DISPATCH[OperatorTag.roll_var] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).var()
)
_DISPATCH[OperatorTag.roll_skew] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).skew()
)
_DISPATCH[OperatorTag.roll_kurt] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).kurt()
)
_DISPATCH[OperatorTag.roll_min] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).min()
)
_DISPATCH[OperatorTag.roll_max] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).max()
)
_DISPATCH[OperatorTag.roll_sum] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).sum()
)
...
_DISPATCH[OperatorTag.roll_cov] = (
    lambda node, c, df: c[0].rolling(int(node.params["n"])).cov(c[1])
)


def _roll_beta_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
    n = int(node.params["n"])
...
def _rank_norm_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
    return c[0].rank(pct=True)


def _winsorize_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
    s = c[0]
...
def _ewm_std_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
    return c[0].ewm(**_ewm_kwargs(node.params)).std()


def _decay_linear_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
    n = int(node.params["n"])
...
_DISPATCH[OperatorTag.gt] = (
    lambda node, c, df: (c[0] > c[1]).astype(float)
)
_DISPATCH[OperatorTag.lt] = (
    lambda node, c, df: (c[0] < c[1]).astype(float)
)
_DISPATCH[OperatorTag.gte] = (
    lambda node, c, df: (c[0] >= c[1]).astype(float)
)
_DISPATCH[OperatorTag.lte] = (
    lambda node, c, df: (c[0] <= c[1]).astype(float)
)
_DISPATCH[OperatorTag.eq] = (
    lambda node, c, df: (c[0] == c[1]).astype(float)
)
_DISPATCH[OperatorTag.crossover] = (
    lambda node, c, df: (
        (c[0] > c[1]) & (c[0].shift(1) <= c[1].shift(1))
    ).astype(float)
)
_DISPATCH[OperatorTag.crossunder] = (
    lambda node, c, df: (
        (c[0] < c[1]) & (c[0].shift(1) >= c[1].shift(1))
    ).astype(float)
)
...
_DISPATCH[OperatorTag.above_zero] = (
    lambda node, c, df: (c[0] > 0).astype(float)
)
_DISPATCH[OperatorTag.below_zero] = (
    lambda node, c, df: (c[0] < 0).astype(float)
)
_DISPATCH[OperatorTag.not_] = (
    lambda node, c, df: (~c[0].astype(bool)).astype(float)
)
...
_DISPATCH[OperatorTag.and_] = (
    lambda node, c, df: (c[0].astype(bool) & c[1].astype(bool)).astype(float)
)
_DISPATCH[OperatorTag.or_] = (
    lambda node, c, df: (c[0].astype(bool) | c[1].astype(bool)).astype(float)
)
...
_DISPATCH[OperatorTag.to_signal] = (
    lambda node, c, df: pd.Series(np.tanh(c[0]), index=c[0].index)
)
_DISPATCH[OperatorTag.scale] = (
    lambda node, c, df: c[0] * float(node.params.get("factor", 1.0))
)


def _combine_handler(
    node: ExpressionNode, c: list[pd.Series], df: pd.DataFrame
) -> pd.Series:
    if "w" in node.params:
...
for _op in (OperatorTag.lag, OperatorTag.diff, OperatorTag.pct_change):
    _LOOKBACK[_op] = lambda node, _op=_op: int(node.params["n"])

# All single-series rolling ops: own lookback = n - 1
for _op in (
    OperatorTag.roll_mean, OperatorTag.roll_std, OperatorTag.roll_var,
    OperatorTag.roll_skew, OperatorTag.roll_kurt, OperatorTag.roll_min,
    OperatorTag.roll_max, OperatorTag.roll_sum, OperatorTag.roll_median,
    OperatorTag.roll_rank, OperatorTag.roll_zscore, OperatorTag.roll_autocorr,
):
    _LOOKBACK[_op] = lambda node, _op=_op: int(node.params["n"]) - 1
...
# Decay
_LOOKBACK[OperatorTag.decay_linear] = lambda node: int(node.params["n"]) - 1
for _op in (OperatorTag.ewm, OperatorTag.ewm_std):
    _LOOKBACK[_op] = lambda node, _op=_op: int(node.params.get("span", 1)) - 1

# Normalization (windowed if "n" in params, else full-series = 0)
for _op in (
    OperatorTag.zscore, OperatorTag.rank_norm, OperatorTag.winsorize,
    OperatorTag.minmax, OperatorTag.demean, OperatorTag.standardize,
):
    _LOOKBACK[_op] = lambda node, _op=_op: int(node.params["n"]) - 1 if "n" in node.params else 0

# Crossover/crossunder: need 1 bar of history for .shift(1)
for _op in (OperatorTag.crossover, OperatorTag.crossunder):
    _LOOKBACK[_op] = lambda node, _op=_op: 1
